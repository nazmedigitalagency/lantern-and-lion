import { ATTENTION_THRESHOLDS } from '../attention/config.ts';
...
// everywhere else in the product). Needs-attention inactivity thresholds
// live in app/lib/attention/config.ts, the one place Feature 14's numbers
// are defined.
...
export function buildNeedsAttention(input: NeedsAttentionInput): { needsAttention: boolean; reasons: string[] } {
  ...
  } else if (inactiveDays >= ATTENTION_THRESHOLDS.INACTIVE_DAYS_TO_FLAG) {
    reasons.push(`No learning activity for ${inactiveDays} day${inactiveDays === 1 ? '' : 's'}.`);
  }
  ...
  if (input.strugglingConceptLabels.length >= ATTENTION_THRESHOLDS.STRUGGLING_CONCEPTS_MIN) {
    reasons.push(`Repeated difficulty with ${input.strugglingConceptLabels.slice(0, 2).join(' and ')} — a little extra practice could help.`);
  }

  if (input.streakEndedRecently && inactiveDays !== null && inactiveDays < ATTENTION_THRESHOLDS.INACTIVE_DAYS_TO_FLAG) {
