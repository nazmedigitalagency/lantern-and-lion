...
const activities = [
  { mark: 'S', title: 'David chooses courage', type: 'Story', time: '8 min', tone: 'gold' },
  { mark: 'W', title: 'Build Psalm 119:105', type: 'Word game', time: '4 min', tone: 'sky' },
  { mark: 'C', title: 'A kind choice at lunch', type: 'Decision', time: '6 min', tone: 'coral' },
  { mark: 'M', title: 'Make a courage card', type: 'Create', time: '10 min', tone: 'teal' },
  { mark: 'T', title: 'Samuel listens', type: 'Trail', time: '7 min', tone: 'cream' },
  { mark: 'Q', title: 'What did Ruth notice?', type: 'Quick quiz', time: '3 min', tone: 'navy' },
];
...
export default function ChildDashboardPage() {
  ...
  useEffect(() => {
    try {
      ...
      if (family?.children?.length) { setChildren(family.children); setActiveId(family.children[0].id); }
  ...
  return <main className={`child-dashboard ${teen ? 'child-dashboard-teen' : ''}`}>
    <header className="child-topbar"><a href="/" className="child-logo"><Image src="/lantern-lion-logo.png" alt="" width={54} height={54} priority /><span><strong>{teen ? 'Lantern After Dark' : 'The Lantern Club'}</strong><small>Lantern &amp; Lion</small></span></a><nav className="child-nav" aria-label="Child dashboard"><button className={view === 'today' ? 'active' : ''} onClick={() => setView('today')}>Today</button><button className={view === 'library' ? 'active' : ''} onClick={() => setView('library')}>Explore</button><button className={view === 'progress' ? 'active' : ''} onClick={() => setView('progress')}>My progress</button></nav><div className="child-header-actions"><button className="help-button" onClick={() => setShowHelp(true)}>Ask for help</button><div className="profile-switch"><button className="profile-button" aria-expanded={showProfiles} onClick={() => setShowProfiles(!showProfiles)}><span>{child.name.slice(0,1)}</span><b>{child.name}</b></button>{showProfiles && <div className="profile-menu">{children.map((item) => <button key={item.id} onClick={() => { setActiveId(item.id); setShowProfiles(false); }}><span>{item.name.slice(0,1)}</span><div><strong>{item.name}</strong><small>{item.age >= 13 ? 'Teen space' : 'Child space'}</small></div></button>)}<a href="/family-setup">Parent manages profiles</a></div>}</div></div></header>
    ...
    {view === 'today' && <div className="child-dashboard-body"><section className="child-welcome"><div><p className="child-kicker">{teen ? 'Your weekly practice' : 'Your next light'}</p><h1>Hi, {child.name}. Ready for one good step?</h1><p>{teen ? 'Pick up your courage study, then choose what you want to explore.' : 'Today, David faces something enormous. See what courage looks like before the battle begins.'}</p><div className="daily-stats"><span><b>{goalDone}/5</b> weekly lights</span><span><b>{points}</b> light points</span><span><b>3</b> day return</span></div></div><div className="welcome-lantern" aria-hidden="true"><span></span><i></i><b>{points}</b></div></section>
      ...
      <section className="continue-layout"><article className="continue-card"><div className="continue-art"><span>01</span><div className="hill hill-one"></div><div className="hill hill-two"></div><i></i></div><div className="continue-copy"><p className="child-kicker">Continue your story</p><h2>{teen ? 'Courage before the crowd' : 'David chooses courage'}</h2><p>{teen ? 'David’s confidence did not come from the crowd. Trace what he remembered before he stepped forward.' : 'David remembers how God helped him before. Then he takes one brave step.'}</p><div className="lesson-progress"><span><i style={{ width: completed.includes('David chooses courage') ? '100%' : '42%' }} /></span><small>{completed.includes('David chooses courage') ? 'Finished' : 'Part 2 of 5'}</small></div><button className="button button-primary" onClick={() => finishActivity('David chooses courage')}>{completed.includes('David chooses courage') ? 'Story finished' : 'Continue the story'}</button></div></article>
    ...
    {view === 'library' && <div className="child-dashboard-body dashboard-view"><div className="dashboard-title"><p className="child-kicker">Explore the club</p><h1>What do you feel like doing?</h1><p>Choose a story, a puzzle or something to make. There’s no endless scroll here.</p></div><div className="dashboard-filters"><button className="active">All activities</button><button>Stories</button><button>Games</button><button>Make something</button></div><div className="dashboard-activity-grid library-grid">{activities.map((item) => <article className={item.tone} key={item.title}><span className="activity-mark">{item.mark}</span><small>{item.type} · {item.time}</small><h3>{item.title}</h3><button onClick={() => finishActivity(item.title)}>{completed.includes(item.title) ? 'Finished' : 'Open activity'}</button></article>)}</div></div>}
    ...
    {showHelp && <div className="help-overlay" role="presentation" onClick={() => setShowHelp(false)}><section className="help-dialog" role="dialog" aria-modal="true" aria-labelledby="help-title" onClick={(event) => event.stopPropagation()}><button className="close-help" aria-label="Close help" onClick={() => setShowHelp(false)}>×</button><p className="child-kicker">You did the right thing</p><h2 id="help-title">What kind of help do you need?</h2><p>A trusted grown-up will see your message. You won’t get in trouble for asking.</p><div><button onClick={() => setShowHelp(false)}>Something feels wrong</button><button onClick={() => setShowHelp(false)}>I’m stuck in an activity</button><button onClick={() => setShowHelp(false)}>I want my parent</button></div><small>If you feel unsafe right now, leave the device and find a trusted grown-up near you.</small></section></div>}
  </main>;
}
